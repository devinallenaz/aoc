...
public class Day10 : Solver
{
    ...
    private static IEnumerable<Point> ReachableNines(int[,] grid, Point current, int currentValue)
    {
        if (currentValue == 9)
        {
            return [current];
        ...
        var reachable = current.AdjacentPointsCardinal()
            .Where(next => grid.ValueOrNull(next.x, next.y) == nextValue)
            .SelectMany(next => ReachableNines(grid, next, nextValue))
            .Distinct();
